...
function CBusNetId(project, network, param3, param4) {
	...
	CBusNetId.validateProjectName(project);
	this.project = project;

	// network
	this.network = cbusUtils.integerise(network);
	CBusNetId.validateNumber(this.network, "network");

	if (param3 === `p`) {
		// unit address
		this.unitAddress = cbusUtils.integerise(param4);
		CBusNetId.validateNumber(this.unitAddress, "unitAddress");
		...
		CBusNetId.validateNumber(this.application, "application");

		this.group = cbusUtils.integerise(param4);
		CBusNetId.validateNumber(this.group, "group");
...
CBusNetId.validateProjectName = function (name) {
...
CBusNetId.validateNumber = function (value, type) {
	if ((typeof value !== `undefined`) && ((value < 0) || (value > 255))) {
		throw new Error(`${type} out of range: ${value}`);
	}
};
