	...
	carrier.carry(this.socket, function (line) {
		if (that.connectionReady) {
			...
			if (parts = line.match(SERVICE_READY_REGEX)) {
				log(`Connected to C-Gate server ${parts[1]} (syntax ${parts[2]})`);
				...
				log(`C-Gate session configured and ready at ${that.cgateControlPort}:${that.cgateIpAddress}`);
...
CBusClient.prototype.receiveLevel = function (netId, callback) {
	const cmd = this._buildGetCommandString(netId, 'level');
	this._sendMessage(cmd, callback);
};

CBusClient.prototype.setBrightness = function (netId, level, callback, delay) {
...
CBusClient.prototype._sendMessage = function (message, callback) {
	...
	this.socket.write(request.raw + `\n`, function (err) {
		if (err) {
			log(`error '${err} when sending '${chalk.green(request.raw)}'`);
		} else {
...
CBusClient.prototype._resolveResponse = function (response) {
	...
	if (response.matched) {
		...
		log(`matched response '${chalk.magenta.underline(response.raw)}' to request '${chalk.magenta.underline(request.raw)}' ` + chalk.dim(`(${this.pendingCommands.size} pending requests)`));
...
CBusClient.prototype._socketReceivedLine = function (line) {
	try {
		...
		log(chalk.red(`rx unparsable line: '${line}', exception: ${err}`));
