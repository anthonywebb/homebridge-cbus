...
CGateDatabase.prototype._patchDevicesToGroups = function () {
	function link(unit, unitId, channelNumber, group, groupId, inferredType) {
		...
		log(`linking ${chalk.red.bold(unit.tag)}/${channelNumber} (${unitId}) as ${inferredType} to ${chalk.red.bold(group ? group.tag : `unknown`)} (${groupId})`);
...
CGateDatabase.prototype._inferGroupTypes = function () {
	_.forEach(this.groups, (group, groupId) => {
		...
		if (numTypes === 0) {
			log(`${chalk.red.bold(group.tag)} ${groupId} isn't directly mapped to a unit.`);
		} else if (numTypes === 1) {
			log(`${chalk.red.bold(group.tag)} ${groupId} looks like a ${chalk.red.bold(types[0])}.`);
			group.inferredType = types[0];
		} else {
			log(`${chalk.red.bold(group.tag)} ${groupId} is mapped to multiple types ${types}.`);
		}
	});
};
