...
function _parseXML(databaseXML) {
	...
	const project = databaseXML.installation.project;
	const projectName = project.address;

	_.forEach(project.network, network => _parseNetwork(network, projectName, result));

	return result;
}

function _parseNetwork(network, projectName, result) {
	...
	_arrayize(network.unit).forEach(srcUnit => {
		...
		function _extractGroupAddresses(unit) {
			...
			if (typeof unitInfo === `undefined`) {
				...
				let numGroups = unitInfo.outputChannels;
				...
				groups = _.map(groups, o => (o === 255) ? null : new CBusNetId(projectName, network.address, unit.application, cbusUtils.integerise(o)));
			}
...
CGateDatabase.prototype.getNetworkEntity = function (netId) {
	...
	console.assert(this.groupMap !== `undefined`, `if we have this.applications, then we should have this.groupMap`);

	// TODO perhaps change to throw?
	if (this.netId.project !== netId.project) {
		throw new Error(`getGroupName unable to search outside default project`);
	}
